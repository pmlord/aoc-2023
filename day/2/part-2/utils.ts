import { Draw, GameStruct, isColor } from "./types.ts";

const colorCountRegex = /(\d+)\s+(red|green|blue)/;

export function initDraw(): Draw {
...
export function getValidGamesFromMax(maxDraw: Draw, games: string[]): number {
  let sum = 0;
...
export function getPowerOfDraw(draw: Draw): number {
  return Object.values(draw).reduce((accum, num, i) => {
    if (i === 0) return num;
    return accum * num;
  }, 0);
}

export function getMinFromGame(game: GameStruct): Draw {
  const min: Draw = initDraw();

  game.draws.forEach((draw) => {
    Object.entries(draw).forEach(([color, num]) => {
      if (!isColor(color)) throw "invalid input";
      if (num > min[color]) min[color] = num;
    });
  });
